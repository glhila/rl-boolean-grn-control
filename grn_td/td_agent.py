...
class TDAgent:
    ...
    def __init__(self, primes, edge_functions, target_values, initial_values, alpha=0.1, gamma=0.9, epsilon=0.1, 
        ...
        self.value_table = {}
        self.db_path = str(RESULTS_DIR / "td_agent_value_table.db")
        self._init_db()
    ...
    def sample_actions_intelligently(self, state, num_samples=100):
        ...
        self.comparison_stats['sampled_actions'] += len(unique_actions[:num_samples])
    ...
    def should_prune_path(self, state, depth=0, max_depth=10):
        ...
        if heuristic_val < 0:  # Very unpromising states
            self.comparison_stats['pruned_states'] += 1
            return True
    ...
    def evaluate_state(self, state):
        ...
        """
        self.comparison_stats['evaluated_states'] += 1
    ...
    def train(self, episodes):
        ...
        for episode in range(episodes):
            ...
            if self.is_terminal_state(self.current_state):
                ...
                self.comparison_stats['episodes_succeeded'] += 1
                self.comparison_stats['terminal_hits'] += 1
                base_success_reward = 10.0
                step_penalty = len(intermediate_states) * 0.1
                final_reward = base_success_reward - step_penalty
                self.success = True
                prev_min = self.comparison_stats['min_stabilization_steps']
                if prev_min is None or steps < prev_min:
                    self.comparison_stats['min_stabilization_steps'] = steps
                    self.comparison_stats['best_assignment'] = {
                        n: action[n] for n in self.controllable_nodes
                    }
                ...
                failure_penalty = -5.0
                self.success = False
                self.comparison_stats['episodes_failed'] += 1
        ...
        self.comparison_stats['execution_time_seconds'] = time.perf_counter() - train_start
    ...
    # ============================================================================
    # Comparison / Debug Summary
    # ============================================================================

    def print_comparison_summary(self):
        print("\n================ TD AGENT SUMMARY ================\n")

        print(f"Controllable Nodes: {len(self.controllable_nodes)}")

        print(f"Search Space Size (2^n): {2 ** len(self.controllable_nodes)}")

        print(f"Successful Assignments: {self.comparison_stats['episodes_succeeded']}")

        best = self.comparison_stats['best_assignment']
        print(f"Best Assignment Found: {best if best is not None else 'N/A'}")

        min_steps = self.comparison_stats['min_stabilization_steps']
        print(f"Minimum Stabilization Steps: {min_steps if min_steps is not None else 'N/A'}")

        elapsed = self.comparison_stats.get('execution_time_seconds')
        if elapsed is not None:
            print(f"Execution Time: {elapsed:.4f}s")
        else:
            print("Execution Time: N/A")

        print(f"Sampled Actions: {self.comparison_stats['sampled_actions']}")

        print(f"Evaluated States: {self.comparison_stats['evaluated_states']}")

        print(f"Pruned States: {self.comparison_stats['pruned_states']}")

        print(f"Terminal Hits: {self.comparison_stats['terminal_hits']}")

        print(f"Successful Episodes: {self.comparison_stats['episodes_succeeded']}")

        print(f"Failed Episodes: {self.comparison_stats['episodes_failed']}")

        print(f"Value Table Size: {len(self.value_table)}")

        print(f"Epsilon Final Value: {self.epsilon:.4f}")

        print(f"Success Rate: "
              f"{self.comparison_stats['episodes_succeeded'] / max(1, self.learning_stats['episodes_completed']):.4f}")
